...
def create_azure_vm(resource_group,
    ...
    if 'error' in az_log:
        ...
        if ssh_success:
            ...
            logging.error("az vm create %s ssh failed or max_inst_sec timeout" %instance_name)
            assert instance_name not in vm_created
            vm_created[instance_name] = False
            return
...
s2_startup_script_path = Path.cwd() / 'scripts' / 'startup_s2.sh'
s4_startup_script_path = Path.cwd() / 'scripts' / 'startup_s4.sh'
s48_startup_script_path = Path.cwd() / 'scripts' / 'startup_s48.sh'
harv_startup_script_path = Path.cwd() / 'scripts' / 'startup_harvest.sh'
...
if init_vms:
    ...
    with open(str(deploy_config_path), 'r') as f:
        ...
        for inv in openwhisk_roles['invokers']:
            ...
            # invokers should not be harvest vm
            assert 'Harvest' not in node_config[node_name]['size']
        ...
        for node_name in node_config:
            ...
            evictable = False
            if node_name != host_node:
                evictable = True
                assert vm_size == 'Standard_D2s_v3' or vm_size == 'Standard_D4s_v3' or vm_size == 'Standard_D48s_v3'
            if vm_size == 'Standard_D2s_v3':
                startup_script_path = s2_startup_script_path
            elif vm_size == 'Standard_D4s_v3':
                startup_script_path = s4_startup_script_path
            elif vm_size == 'Standard_D48s_v3':
                startup_script_path = s48_startup_script_path
            else:
                startup_script_path = harv_startup_script_path
...
if azure_deploy_openwhisk:
    ...
    azure_wsk_pulled_action_image_cmd += 'mv runtimes_synthetic.json runtimes.json'
    ssh(destination=username +'@' + external_ips[host_node],
    ...
    azure_reg_func_cmd += 'python3 register_synthetic_actions.py'
    ssh(destination=username +'@' + external_ips[host_node],
...
scp(source= azure_scripts_dir / 'run_harv_synthetic.py', 
    target=username + '@' + external_ips[host_node] + ':~/', 
    identity_file=str(rsa_private_key))
...
# copy trace to host
scp(source= Path.cwd() / 'function_traces' / 'func_trace_synthetic.json', 
    target=username + '@' + external_ips[host_node] + ':~/', 
    identity_file=str(rsa_private_key))