...
parser.add_argument('--dateback-s', dest='dateback_s', type=int, required=True)
...
def get_activation_since(since, limit=50000):
    ...
    res = requests.post(url=DB_PROTOCOL + '://' + DB_HOST + ':' + DB_PORT + '/' + 'whisk_local_activations/_find',
                        json={
                            ...
                            "fields": ['activationId', 'annotations', 'duration', 'end', 'name', 'namespace', 'start'],
                            "limit": limit
                        },
...
activations = get_activation_since(int(time.time()*1000) - dateback_s*1000)
