...
s2_vm_size = 'Standard_D2s_v3'
s4_vm_size = 'Standard_D4s_v3'
s48_vm_size = 'Standard_D48s_v3'
...
parser.add_argument('--vm-size', dest='vm_size', type=str, required=True)
...
assert args.vm_size == 's2' or args.vm_size == 's4' or args.vm_size == 's48'
invoker_vm_cpus = int(args.vm_size.replace('s', ''))
print('invoker_vm_cpus = ', invoker_vm_cpus)
if args.vm_size == 's2':
    invoker_vm_size = s2_vm_size
    startup_script_path = s2_startup_script_path
if args.vm_size == 's4':
    invoker_vm_size = s4_vm_size
    startup_script_path = s4_startup_script_path
elif args.vm_size == 's48':
    invoker_vm_size = s48_vm_size
...
est_harv_vm_vcpus = invoker_vm_cpus
...
while time.time() - start_time <= exp_time:
    ...
    if cur_time - prev_check_time < check_interval:
        ...
        _, live_vm_vcpus = check_cluster_vcpu(external_ips=external_ips, 
            invoker_config=invoker_config, 
            username=username, identity_file=rsa_private_key)
        cluster_vcpus = len(live_vm_vcpus) * invoker_vm_cpus
